...
namespace Desktop.Express.Physics
{
    class ParticleAARectangleCollision : CollisionAlgorithm<IParticleCollider, IAARectangleCollider>
    {
        ...
        public Vector2 CalculateRelaxDistanceBetween(IParticleCollider particle, IAARectangleCollider aaRectangle)
        {
            ...
            nearestVertex += new Vector2(halfWidth, halfHeight);

            // Calculate overlap with all sides.
            float leftDifference = (nearestVertex.X - halfWidth) - (particle.Position.X + particle.Radius);
            if (leftDifference > 0) return relaxDistance;

            float rightDifference = (particle.Position.X - particle.Radius) - (nearestVertex.X + halfWidth);
            if (rightDifference > 0) return relaxDistance;

            float topDifference = (nearestVertex.Y - halfHeight) - (particle.Position.Y + particle.Radius);
            if (topDifference > 0) return relaxDistance;

            float bottomDifference = (particle.Position.Y - particle.Radius) - (nearestVertex.X + halfHeight);
            ...
            if (particle.Position.X < nearestVertex.X - halfWidth)
            {
                nearestVertex.X -= halfWidth;
            }
            else if (particle.Position.X > nearestVertex.X + halfWidth)
            {
                nearestVertex.X += halfWidth;
            }
            else
            ...
            if (particle.Position.Y < nearestVertex.Y - halfHeight)
            {
                nearestVertex.Y -= halfHeight;
            }
            else if (particle.Position.Y > nearestVertex.Y + halfHeight)
            {
                nearestVertex.Y += halfHeight;
            }
            else
